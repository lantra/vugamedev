...
class Fighter:
    ...
    def __init__(self, hp, defense, power, death_function=None, player = False):
        ...
        #calculate xp values if object is not player
        if not player:
            self.xp = self.max_hp + 30 + (10 * (self.defense * self.power)) 
        else:
            self.xp = 0 
...
def random_monster_death_message(): #monster_type
    ...
    x = libtcod.random_get_int(0, 0, 4)
    if x == 0:
        return "splatters!"
    elif x == 1:
...
#this is where we decide the chance of each monster or item appearing.
    ...
    monster_chances = {}
    monster_chances['orc'] = 70  #orc always shows up, even if all other monsters have 0 chance
    monster_chances['elf_savage'] = from_dungeon_level([[60, 1], [30, 3], [15, 4], [0,5]])
    ...
    for i in range(num_monsters):
        ...
        if not is_blocked(x, y):
            ...
            if choice == 'orc':
                ...
                #create a troll
                fighter_component = Fighter(hp=30, defense=2, power=8, death_function=monster_death)
                ai_component = BasicMonster()
            ...
            elif choice == 'elf_savage':
                #create an elf savage!
                fighter_component = Fighter(hp=10, defense = 0, power=3, death_function=monster_death)
                ai_component = BasicMonster()

                fighter_component1 = Fighter(hp=10, defense = 0, power=3, death_function=monster_death)
                ai_component1 = BasicMonster() #not sure why but these need to be repeated

                monster = Object(x, y, 'e', 'elf savage', libtcod.yellow, blocks = True,
                    fighter=fighter_component, ai=ai_component)
                

                #redefine x, y for just this monster
                x1 = libtcod.random_get_int(0, room.x1+1, room.x2-1)
                y1 = libtcod.random_get_int(0, room.y1+1, room.y2-1)

                monster2 = Object(x1, y1, 'e', 'elf savage', libtcod.yellow, blocks = True,
                    fighter=fighter_component1, ai=ai_component1)
                
                
                objects.append(monster2) #append monster 2 here
...
def new_game():
    ...
    fighter_component = Fighter(hp=100, defense=1, power=4, death_function=player_death, player=True)
